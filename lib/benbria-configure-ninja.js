...
(function() {
  var collectCoffeeFiles, commander, config, edgeMapping, factories, findCommand, findLocalCommand, findScript, fp, fs, getOptionString, getOptions, glob, globule, ld, log, makeAssetEdges, makeCommonRules, makeFingerprintEdge, makeLintEdges, makeNinja, makeSimpleRule, makeSourceEdges, makeSystemEdges, ninjaBuilder, packageJson, path, simpleMapOpt, warnMessage, _ref;
  ...
  makeCommonRules = function(ninja, options) {
    ...
    return makeSimpleRule(ninja, {
      name: 'fingerprint',
      command: "$buildCoffee " + (findScript("fingerprint.coffee", config)) + " $cliOptions -b $basePath -o $out $in",
      display: '$basePath'
    });
  };
