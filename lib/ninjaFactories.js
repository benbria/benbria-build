...
(function() {
  ...
  defineFactory = exports.defineFactory = function(name, factory) {
    factory.name = name;
    exports.factories[name] = factory;
  ...
  defineFactory("sass", {
    assignments: function(ninja, config) {
      return ninja.assign('sass', 'vendor/gem-bin/sass');
    },
    makeRules: function(ninja, config) {
      return ['debug', 'release'].forEach(function(releaseType) {
        var cli;
        cli = '$sass --compass $in $out';
        cli += releaseType === 'release' ? ' --style compressed' : ' --sourcemap';
        cli += " && ruby " + (findScript("sass-dep.rb", config)) + " $in > $out.d";
        return makeAssetRule(ninja, 'sass', releaseType, cli);
      });
    },
